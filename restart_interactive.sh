...
FRONTEND_DIR="$BASE_DIR/0210_ETH_Dashboard/frontend"
...
# 2. 启动 Frontend (在新窗口)
cat <<EOF > /tmp/start_frontend.sh
#!/bin/bash
echo "🌐 Dashboard Frontend Starting..."
cd "$FRONTEND_DIR"
# 如果没有 node_modules 则自动安装
...
open -a Terminal /tmp/start_frontend.sh

# 3. 策略控制台 (监视由 Backend 管理的策略输出)
